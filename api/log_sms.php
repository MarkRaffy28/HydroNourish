...
/**
 * ESP32 API - Log SMS Status
 * Updates status of manual alerts or logs new auto-sent alerts.
 */

try {
    // Support both JSON (file_get_contents) and Form Data ($_POST)
    $input = json_decode(file_get_contents('php://input'), true) ?: $_POST;
    
    $id      = isset($input['id']) ? intval($input['id']) : 0;
    $msg     = isset($input['message']) ? $input['message'] : '';
    $to      = isset($input['recipients']) ? $input['recipients'] : '';
    $outcome = isset($input['status']) ? $input['status'] : 'Sent'; // 'Sent' or 'Failed'
    ...
    if ($id > 0) {
        // Update status of an existing message (sent from web UI)
        ...
        // Log a new SMS sent automatically by the ESP32 (e.g., Tank Low alert)
